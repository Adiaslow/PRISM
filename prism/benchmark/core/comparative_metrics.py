"""Metrics for comparing molecular graph matching algorithms."""
...
class ComparativeMetrics(BaseModel):
    """Metrics for comparing algorithm performance on molecular subgraph isomorphism problems."""
    ...
    common_substructure_ratio: float = (
        0.0  # Average ratio of match size to max common size
    )
    ...
    def from_results(
        ...
        Returns:
            ComparativeMetrics calculated from the results
        """
        ...
        common_ratios = [
            match_size / max_common_size if max_common_size > 0 else 0.0
        ...
        for range_name, (min_size, max_size) in size_ranges.items():
            ...
            if range_results:
                ...
                metrics.by_size_range[range_name] = {
                    "avg_match_size": float(
...
class ComparativeTestResult(BaseModel):
    """Result of a single comparative benchmark test for molecular graph matching."""
    ...
    max_common_size: Optional[int] = (
        None  # Estimate of maximum common size (if available)
    )
    ...
    match_ratio: float = 0.0  # Ratio of match size to minimum molecule size
